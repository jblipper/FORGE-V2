...
def run(pathin,pathout,res,run_year,phi_v,phi_a,eG_cst,eH_cst,c_dep,q,filein_pre,fileout,parameters):
  ...
  varlist=['lon','lat','time',\
          't2m','prec','GPP'] #the model takes three data variables from the input file, two-metre temperature (t2m) K or C, mean annual precipitation (prec) in mm/year, and Gross Primary Production (GPP)
### variable name used in the FORGE model,corresponding to the names from the input
  varuse =['', '',  '',           \
          't2m','prec','in_bm_hum']  #variables from input are renamed for use in the FORGE model

  
#Model Parameters
  Nani=1 # only one animal functional type
  Nhum=1 # just one human funtional type
  CtoDM=0.45  # conversion factor between dry mass and carbon mass
  edi_frac=0.3 # assume 30% of NPP is assumed to be accessible to animals
  ani_edi=0.28 # conversion between live weight and dry mass after excluding water and bones for the animal food
  decay_frac=0.5 # assume 50% of fresh biomass available to herbivores decays each day
  lit_prop=0.5 # assume the amount of dead biomass (litter) generated per day is 50% of NPP
  decay_lit_frac=0.3 # assume 30% of dead biomass (litter) decays per day 
  gpp_to_npp=0.7 #proportion of GPP that is not used for plant respiration and contributes to NPP
  phinpp = 0.7  # fraction of npp available to animals by biome  
...
'hum_popu','fat_hum','expend_hum','intake_veg','estab_hum','starv_hum','estab_actual_hum','mort_starv_hum','food_veg','tforage','tG','eG','eH','intake_ani','food_ani','tH','benifit_veg','benifit_ani','est_date_hum','est_elapse_hum','influx_v'] #list of variables to be tracked in short form
  ...
  for loop in range(run_year):
    ...
    phinpp= phinpp*np.ones((1,nlats,nlons))

    npp= varin['in_bm_hum']*gpp_to_npp/(1000. * CtoDM)*phinpp # influx to the edible biomass for herbivores, unit: kgDM/m2/day
    ...
    influx_bm_avail_Nani=np.zeros((365,Nani,nlats,nlons)) #initializes array for daily influx of fresh biomass available to herbivores

    influx_bm_avail_Nani[:,0,:,:]=npptotal[:,:,:]*edi_frac #assumes that the daily influx of fresh biomnass available to herbivores is a constant fraction of npp 
    ...
    influx_veg = (npp*0.065+npp*0.5*fBNPP) * phi_v
    ...
    ani_tmp1 = ani_popu_0[0] * 180.*ani_edi *phi_a
    #ani_tmp1= ani_popu_0[0] * fgrass_yr*180.*ani_edi * phi_a
    #ani_tmp2= ani_popu_0[1] * ftree_yr *180.*ani_edi * phi_a
    ani_DM_0 = ani_tmp1 # sum up the density on tree and grass fractional covers to derive the density for the whole grid cell
    ani_old = ani_DM_0 *1
