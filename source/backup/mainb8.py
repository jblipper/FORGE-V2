...
def run(pathin,pathout,res,run_year,ninput_years,filein_pre,fileout,parameters):
...
# This section contains all parameters used to quantitify the interactions being modeled in the FORGE simulation. Their values should be updated based on new publications or for sensitivity tests. They may be replaced with elements of the list given a positional parameter to the 'main.py' module for the script 'run.sh' that performs several similtaneous runs with varied parameters.
  ...
  decay_frac=0.01 # proportion of fresh biomass available to herbivores which decays each day
  ...
  gpp_to_npp=1 #proportion of GPP that is not used for plant respiration and contributes to NPP
  phinpp = 0.9  # proportion of npp available to animals 
  decay_frac_veg_food_hum = 0.01 # proportion of of plant food edible to humans that decays each day 
