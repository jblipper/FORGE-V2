...
def write_var(res,pathin,pathout,filein,fileout,nlat,nlon,ntime,Nveg,Nani,Nhum,outlist,varout,shortname,mod_pars,run_pars):
  outdir=pathout+fileout+'-dir'
  if not os.path.exists(outdir):
    os.mkdir(outdir) 
  ncref=Dataset(pathin+filein,'r') # opens input netCDF file
  nc=Dataset(outdir+'/'+fileout,'w',format='NETCDF3_CLASSIC') # createss output netCDF file
  ...
  for loop in range(len(outlist)): #loops through list of state variables output from simulation
      ...
      for par, value in mod_pars.items():
           nc.setncattr(par,value)
      for par, value in run_pars.items():
           nc.setncattr(par,value) 
  #####################################################################
  ncref.close() # closes input netCDF file
  nc.close() # closes output netCDF file
  
  outfile_path = os.path.join(outdir, 'parameters.txt')
  with open(outfile_path, 'w') as file:
       file.write("Model Parameters:\n")
       for par, value in mod_pars.items():
            file.write(f'{par}: {value}\n')
       file.write("Running Parameters\n")
       for par, value in run_pars.items():
            file.write(f'{par}: {value}\n')
